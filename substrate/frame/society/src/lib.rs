...
//! * `propose_deposit` - A member can propose a vote for the Society to place a deposit on a referendum.
//! * `vote_for_deposit` - A member can vote to approve or reject placing a deposit on a referendum.
//!
//! #### For Super Users
//!
...
//! ### Deposit Voting
//!
//! The Society pallet includes functionality for members to participate in
//! decision-making regarding the placement of deposits on referenda. This process
//! allows the society to act as a collective entity in the broader governance system.
//!
//! #### Process
//!
//! 1. **Proposal**: Any member can propose that the society place a deposit on a referendum.
//! 2. **Voting**: Members vote on the proposal, with votes weighted by their rank.
//! 3. **Execution**: If the vote passes, the society places the deposit on the referendum.
//!
...
pub use pallet::*;
use pallet_democracy::{self, Config as DemocracyConfig};
...
pub mod pallet {
	...
	pub trait Config<I: 'static = ()>: frame_system::Config {
        ...
        /// The Referenda pallet instance
        type Referenda: PlaceDeposit<Self::AccountId>;

        /// The period for which a deposit vote remains active.
		#[pallet::constant]
        type DepositVotingPeriod: Get<Self::BlockNumber>;

<<<<<<< HEAD
=======
        /// The safety margin before the referendum deposit period ends
        #[pallet::constant]
        type DepositVotingSafetyMargin: Get<Self::BlockNumber>;

>>>>>>> 065c5065d75 (add dd for society)
	...
	pub enum Error<T, I = ()> {
		...
		NoVotes,
        /// The deposit vote has expired.
        DepositVoteExpired,
<<<<<<< HEAD
=======
        /// The referendum is not in a state where it needs a deposit.
        InvalidReferendumState,
        /// A deposit proposal for this referendum already exists.
        AlreadyProposed,
        /// There's not enough time left to complete voting process
        InsufficientTime,
>>>>>>> 065c5065d75 (add dd for society)
	}
	...
	pub enum Event<T: Config<I>, I: 'static = ()> {
        ...
        /// A vote for placing a deposit has been cast.
        DepositVoteCast { member: T::Accountid, referendum_index: ReferendumIndex, vote: Vote },
        /// A deposit vote has passed and the deposit has been placed.
        DepositPlace { referendum_index: ReferendumIndex },
<<<<<<< HEAD
=======
        /// A new deposit proposal has been made.
        DepositProposed { referendum_index: ReferendumIndex, proposer: T::AccountId },
>>>>>>> 065c5065d75 (add dd for society)
	}
...
<<<<<<< HEAD
    /// Trait to place deposit on referenda
    pub trait PlaceDeposit<AccountId> {
        fn place_decision_deposit_as_society(referendum_index: ReferendumIndex) -> DispatchResult;
=======
    /// Trait for interacting with the Referenda pallet
    pub trait PlaceDeposit<AccountId> {
        fn place_decision_deposit_as_society(referendum_index: ReferendumIndex) -> DispatchResult;
        fn referendum_status(referendum_index: ReferendumIndex) -> Result<ReferendumStatus, DispatchError>;
    }

    /// Possible state for Referendum
    /// Use from governance pallet instead?
...
>>>>>>> 065c5065d75 (add dd for society)
    ...
    /// Storage for votes on deposit
    #[pallet::storage]
    pub type DepositVotes<T: Config<I>, I: 'static = ()> = StorageDoubleMap<
        _,
        Blake2_128Concat,
        ReferendumIndex,
        Blake2_128Concat,
    ...
    /// Storage for decision deposit vote expiry
    #[pallet::storage]
    pub type DepositVotesExpiry<T: Config<I>, I: 'static = ()> = StorageMap<
        _,
        Blake2_128Concat,
        ReferendumIndex,
        T::BlockNumber,
    ...
    /// Storage for decision deposit tallies
    #[pallet::storage]
    pub type DepositTallies<T: Config<I>, I: 'static = ()> = StorageMap
        _,
        Blake2_128Concat,
        ReferendumIndex,
        Tally,
	...
	impl<T: Config<I>, I: 'static> Hooks<BlockNumberFor<T>> for Pallet<T, I> {
		fn on_initialize(n: BlockNumberFor<T>) -> Weight {
            ...
            // Check for expired deposit votes
            DepositVotesExpiry::<T, I>::iter(|(referendum_index, expiry)| {
                if n >= expiry {
                    if let Ok(()) = Self::finalize_deposit_vote(referendum_index) {
                        weight = weight.saturating_add(T::WeightInfo::finalize_deposit_vote());
                    }
                }
            });
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
        ...
        /// Allow members of the society to participate in the decision-making process
        /// for placing deposits on referenda. Each member's vote is weighted based on their rank
        /// within the society.
        #[pallet::call_index(20)]
        #[pallet::weight(T::WeightInfo::vote_for_deposit())]
        pub fn vote_for_deposit(
            ...
            let mut tally = Self::get_deposit_tally(referendum_index);

            let first_time = DepositVotes::<T, I>::mutate(referendum_index, &voter, |v| {
                let first_time = v.is_none();
                *v = Some(Self::do_vote(*v, approve, record.rank, &mut tally));
                first_time
            });

            // Update the tally
            DepositTallies::<T, I>::insert(referendum_index, tally);

            // Only set expiry if it doesn't exist yet
            if !DepositVotesExpiry::<T, I>::contains_key(referendum_index) {
                let expiry = frame_system::Pallet::<T>::block_number() + T::DepositVotingPeriod::get();
                DepositVotesExpiry::<T, I>::insert(referendum_index, expiry);
            }

            Self::deposit_event(Event::<T, I>::DepositVoteCast {
                referendum_index,
                voter,
                vote: approve
            });
...
=======
        ...
        /// Allow society members to propose to place a decision deposit on a referendum.
        #[pallet::call_index(21)]
        #[pallet::weight(T::WeightInfo::propose_deposit())]
        pub fn propose_deposit(
            ...
            ensure!(Members::<T, I>::contains_key(&proposer), Error::<T, I>::NotMember);

            let referendum_status = T::Referenda::referendum_status(referendum_index)?;
            ensure!(
                matches!(referendum_status, ReferendumStatus::NeedsDeposit { .. }),
                Error::<T, I>::InvalidReferendumState
                );

            ensure!(
                !DepositVotesExpiry::<T, I>::contains_key(referendum_index),
                Error::<T, I>::AlreadyProposed
                );

            let referendum_end = match referendum_status {
                ReferendumStatus::NeedsDeposit { deposit_end, .. } => deposit_end,
                _ => unreachable!(),
            };

            let current_block = frame_system::Pallet::<T>::block_number();
            let available_time = referendum_end.saturating_sub(current_block);
            let voting_period = T::DepositVotingPeriod::get();
            let safety_margin = T::DepositVotingSafetyMargin::get();

            ensure!(
                available_time > voting_period + safety_margin,
                Error::<T, I>::InsufficientTime
                );

            let expiry = current_block.saturating_add(voting_period);
            DepositVotesExpiry::<T, I>::insert(referendum_index, expiry);

            DepositTallies::<T, I>::insert(referendum_index, Tally::default());

            Self::deposit_event(Event::<T, I>::DepositProposed {
                referendum_index,
                proposer
            });
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
    ...
    /// Returns the current tally of votes on a specific deposit referendum.
    fn get_deposit_tally(referendum_index: ReferendumIndex) -> Tally {
        DepositTallies::<T,I>::get(referendum_index)
    }

    fn finalize_deposit_vote(referendum_index: ReferendumIndex) -> DispatchResult {
        let tally = Self::get_deposit_tally(referendum_index);
        if tally.approve > tally.reject {
            T::Referenda::place_decision_deposit_as_society(referendum_index)?;
            Self::deposit_event(Event::<T, I>::DepositPlaced { referendum_index });
        }
        // Clean up storage regardless of the outcome
        DepositVotes::<T, I>::remove_prefix(referendum_index, None);
        DepositVotesExpiry::<T, I>::remove(referendum_index);
        DepositTallies::<T, I>::remove(referendum_index);
        Ok(())
    }
}
